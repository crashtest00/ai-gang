...
(REQ-08 — the initialization agent follows it directly, unchanged) or has
been converted to a named graph (REQ-09). This satisfies REQ-09's
acceptance: "A migration-status record lists every phase... with exactly
one of the two states ('as-is'/'converted: `<graph_id>`'), and the
Cloudflare phase is the first to show 'converted.'"

Per REQ-09, conversion proceeds one **conditional** phase at a time and does
...
| **Phase 1.0/1.1: Jira Instance Setup** | **Yes**, as of `escalation` (REQ-17) — see "Reclassified this pass" below | **converted: `jira-instance-setup`** (`setup/graphs/jira-instance-setup.graph.yaml`) |
...
| **Phase 2.5: Beta VM Bootstrap** | **Yes**, as of `escalation` (REQ-17) — Beta's own-VM-vs.-container-on-Dev fork; see "Reclassified this pass" below | as-is, not yet converted |
...
## Reclassified this pass — the `escalation` node kind (REQ-17)

Before `escalation` existed, this record's only branching mechanism was
`decision` (REQ-02), which strictly tests real, observable environment
...
state"). `escalation` (REQ-17) closes that gap for a human-resolved fork,
...
The product owner named "Jira-vs-local mode" (`canonical-work-model.md`
REQ-14) as a candidate. Read literally, that decision does not belong here
at all: REQ-14 explicitly bars selecting Jira mode during a project's own
initialization — "Jira mode MUST NOT be selectable during project
initialization; a project MUST complete initialization in local mode and
connect Jira, if at all, as a separate, later operation" — so no per-project
mode switch is ever reached while walking Phase 0-4. `docs/ClaudeInstructions.md`
confirms this independently: nowhere in the document does "Jira mode" or
"local mode" appear, and Phase 0's question set doesn't ask about Jira at
all. Forcing REQ-14's project-level switch into Phase 1 as an escalation
node would misrepresent both documents.
...
environment state to probe — unrepresentable before REQ-17, cleanly
representable now.

This is deliberately **not the same decision** as REQ-14's per-project
...
instance, gating whether Phase 1 runs at all), REQ-14's is project-scoped
(always local at init; Jira connects later, per project, via REQ-15).
...
pass** — REQ-09's one-conditional-phase-at-a-time budget went to Phase 1
...
is a follow-on candidate for whichever pass picks up REQ-09's next
conditional phase.

## Cross-cutting branch-point classes (not tied to one numbered phase)

These generalize beyond a single phase, per REQ-12/REQ-13:
